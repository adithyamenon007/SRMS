...
<div class="container-fluid">
	<center>
		<h3 style="color: green;"><span class="glyphicon glyphicon-thumbs-up"></span> The Toppers <span class="glyphicon glyphicon-thumbs-up"></span></h3>
	</center>
</div>

<?php
$con = mysqli_connect("localhost","root","","SRMS") or die(mysqli_error($con));
$fail_query = "SELECT name,class,roll_no,pm,cm,mm,((pm+cm+mm)/3)*100/30 AS percent FROM students WHERE ((pm+cm+mm)/3)*100/30 = (SELECT max(((pm+cm+mm)/3)*100/30) FROM students)";
...
<div class="container extra">
	<?php if($num_fails>0)
		...
		?>
		<center>
			<h4 style="color: green;">No Toppers</h4>
		</center>
		<?php }
		?>
</div>
